...
$me = mysqli_query($link, "SELECT * FROM historias WHERE n_vistas>0 AND id_user='".$_SESSION['login'][2]."'");
...
<head>
	...
	<style type="text/css">
		fieldset{width:100%;text-align:left;font-family:Lato,sans-serif}form{border-top:1px solid #c8c7cc;border-bottom:1px solid #c8c7cc}legend{margin:0 0 5px;color:#6f6f74;font-size:13px;line-height:1.4}.item{position:relative;box-sizing:border-box;background:#fff}.item p{border-top:1px solid #c8c7cc;margin:0 0 0 8px;height:44px;color:#000;font-size:15px;line-height:44px}.item:nth-child(1) p{border:0}input[type=checkbox]{display:none}.toggle{position:absolute;top:0;bottom:0;right:9px;margin:auto;width:51px;height:31px}.toggle i,.toggle label{box-sizing:border-box;display:block;background:#fff}.toggle label{width:51px;height:32px;border-radius:32px;border:2px solid #e5e5e5;transition:all .3s ease}.toggle i{position:absolute;top:2px;left:2px;width:28px;height:28px;border-radius:28px;box-shadow:0 0 1px 0 rgba(0,0,0,.25),0 3px 3px 0 rgba(0,0,0,.15);background:#fff;transition:all .3s cubic-bezier(.275,-.45,.725,1.45)}input[type=checkbox]:active+.toggle i{width:35px}input[type=checkbox]:active+.toggle label,input[type=checkbox]:checked+.toggle label{border:16px solid #4cd964}input[type=checkbox]:checked+.toggle i{left:21px}input[type=checkbox]:checked:active+.toggle label{border:16px solid #e5e5e5}input[type=checkbox]:checked:active+.toggle i{left:14px}
	</style>
</head>
...
<body style="width: 100% !important; height: 100% !important;" id="body_g">
	...
	<div id="change_g2" style="width: 100% !important; height: 100% !important">
      ...
      //STATS
				...
				$query_ranking = mysqli_query($link, "SELECT DISTINCT id_user FROM `historias` ORDER BY n_vistas DESC");
				$rank_conta = 0;
				while ($info_ranking = mysqli_fetch_array($query_ranking)) {
					$id_ranking_user = $info_ranking['id_user'];
					if ($id_user == $id_ranking_user) {
						$ranking = $rank_conta;
					}
				...
				$last_story_query = mysqli_query($link, "SELECT data FROM `historias` WHERE id_user='$id_user' ORDER BY data DESC LIMIT 1");
				$info_last_data = mysqli_fetch_assoc($last_story_query);
				$last_story_usa = $info_last_data['data'];

				$last_story = date("d/m H:i", strtotime($last_story_usa));
				?>
				<h1 class="h1_menu"><i class="far fa-chart-bar"></i> Statistics</h1>
				...
				<div class="text-center">
					...
					<div style="display: inline-block; vertical-align: top;">
						...
						<h2 class="h1_menu Neue-bold" style="font-size: 98% !important; line-height: 50px !important;">@<?php echo $username; ?></h2>
					</div>
					<div style="display: inline-block; vertical-align: top;">
				...
				<div class="text-center" style="height: 100%; width: 100%;background: rgba(255, 255, 255, 0.8); padding-top: 1%;">
					...
					<div class="card text-white bg-info mb-3" style="width: 47%; margin: 1%; display: inline-block;">
						<div class="card-header Neue-bold">Average Viewers</div>
					...
					<div class="card text-white bg-dark mb-3" style="width: 47%; margin: 1%; display: inline-block;">
						<div class="card-header Neue-bold">Total Views</div>
						<div class="card-body" style="font-family: Lato;">
					...
					<div class="card text-white bg-success mb-3" style="width: 47%; margin: 1%; display: inline-block;">
						<div class="card-header Neue-bold">Total Stories</div>
						<div class="card-body" style="font-family: Lato;">
					...
					<div class="card text-white bg-danger mb-3" style="width: 47%; margin: 1%; display: inline-block;">
						<div class="card-header Neue-bold">Unique Viewers</div>
						<div class="card-body" style="font-family: Lato;">
							<h5 class="card-title"><?php echo $n_viewers_unicos; ?></h5>
						</div>
					</div>
					<div class="card text-white bg-primary mb-3" style="width: 47%;margin: 1%;display: inline-block;">
						<div class="card-header Neue-bold">Ranking</div>
						<div class="card-body" style="font-family: Lato;">
							<h5 class="card-title"><?php echo "#".$ranking; ?></h5>
						</div>
					</div>
					<div class="card text-white bg-secondary mb-3" style="width: 47%;margin: 1%;display: inline-block;">
						<div class="card-header Neue-bold">Last Story Posted</div>
						<div class="card-body" style="font-family: Lato;">
							<h5 class="card-title" style="font-size: 110%;"><?php echo "".$last_story; ?></h5>
						</div>
					</div>
					<canvas id="me" style="background-color: #f9eded;"></canvas>
				</div>
			...
			<section class="two" onclick="goToPage(1)">
				<h1 class="h1_menu"><i class="fas fa-user-cog"></i> Settings</h1>
				<hr style="margin-top: 0;">
				<div class="text-center">
					<div style="display: inline-block; vertical-align: top;">
						<span class="h1_menu" style="line-height: 50px !important"><?php echo $followers; ?></span>
					...
					<fieldset>
						...
						<form action="">
							...
							<div class="item">
								...
								<input type="checkbox" id="toggle_calendar_day_view" name="" value="" checked>
								<div class="toggle">
							...
							<div class="item">
								...
								<input type="checkbox" id="toggle_reminders" name="" value="" checked>
								<div class="toggle">
			...
			<section class="three" onclick="goToPage(2)">
				<h1 class="h1_menu"><i class="far fa-eye"></i> Top Stalkers</h1>
				<hr style="margin-top: 0;">
				<div class="text-center">
					<div style="display: inline-block; vertical-align: top;">
						<span class="h1_menu" style="line-height: 50px !important"><?php echo $followers; ?></span>
			...
			<section class="four" onclick="goToPage(3)" style="overflow-y: scroll; background-color: #006ce0;">
				<h1 class="h1_menu"><i class="fas fa-home"></i> Home</h1>
				<hr style="margin-top: 0;">
				<div class="text-center">
					<div style="display: inline-block; vertical-align: top;">
						<span class="h1_menu" style="line-height: 50px !important"><?php echo $followers; ?></span>
